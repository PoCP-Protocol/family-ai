      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.header}>
            <View style={styles.topBar}><Pressable accessibilityRole="button" accessibilityLabel="返回" onPress={() => router.back()} style={styles.topBack}><IconSymbol name="chevron.left" size={26} color="#22272D" /></Pressable><Text style={styles.topTitle}>活动详情</Text><Text style={styles.topMore}>↗</Text></View>
            {projectionState === "error" ? (
              <Pressable accessibilityRole="button" accessibilityLabel="重新加载活动详情" onPress={loadSurfaces} style={styles.inlineNotice}>
                <Text style={[styles.inlineNoticeText, { color: colors.muted }]}>暂时无法读取活动详情，点击重试</Text>
              </Pressable>
            ) : null}
        ...
        ListFooterComponent={
          <View style={styles.footer}>
            ...
            <View style={styles.actionRow}><Pressable accessibilityRole="button" accessibilityLabel="查看我的活动" onPress={() => router.push("/ui/UI-24" as Href)} style={({ pressed }) => [styles.consultAction, { borderColor: colors.tint }, pressed && styles.pressed]}><Text style={[styles.consultText, { color: colors.tint }]}>我的活动</Text></Pressable><Pressable accessibilityRole="button" accessibilityLabel={saved ? "已记下报名意向" : "立即报名"} disabled={saveState === "submitting"} onPress={saveInterest} style={({ pressed }) => [styles.primaryAction, pressed && styles.pressed, saveState === "submitting" && styles.disabled]}>{saveState === "submitting" ? <View style={styles.loadingContent}><ActivityIndicator size="small" color="#FFFFFF" /><Text style={styles.primaryText}>正在保存</Text></View> : <Text style={styles.primaryText}>{saved ? "已记下报名意向" : "立即报名"}</Text>}</Pressable></View>
            <Modal transparent visible={saveState === "saved"} animationType="fade" onRequestClose={() => setSaveState("idle")}><View style={styles.modalScrim}><View style={styles.successModal}><IconSymbol name="checkmark.circle.fill" size={44} color={colors.success} /><Text style={styles.successTitle}>活动意向已保存</Text><Text style={styles.successText}>已记在家庭私有空间；当前没有报名、占用名额或发送通知。</Text><Pressable accessibilityRole="button" accessibilityLabel="我知道了" onPress={() => setSaveState("idle")} style={styles.successAction}><Text style={styles.successActionText}>我知道了</Text></Pressable></View></View></Modal>
          </View>
        }
      />
    </ScreenContainer>
