/**
 * Legacy mobile-template compatibility facade.
 *
 * Family AI never calls a model provider from the mobile companion server.
 * All production AI capabilities must pass through the canonical NestJS
 * FAMILY_LLM_GATEWAY, where family scope, consent, policy and audit are
 * enforced. Callers of this legacy facade therefore fail closed and may use
 * their explicitly labelled deterministic fallback.
 */
...
export type ModelCatalog = { data: Array<{ id: string; object?: string; owned_by?: string }> };

export async function listLLMModels(): Promise<ModelCatalog> {
  return { data: [] };
}

export async function invokeLLM(_params: InvokeParams): Promise<InvokeResult> {
  throw new Error("FAMILY_LLM_GATEWAY_REQUIRED");
}
