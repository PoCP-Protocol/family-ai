...
import {
  LEGACY_FAMILY_SURFACE_LOOPS,
  LEGACY_FAMILY_BUSINESS_SCENARIOS,
  LEGACY_FAMILY_UI_ARCHITECTURE_BINDINGS,
  UI01_HOME_FEATURES,
  UI01_ENTRY_EXECUTION_QUEUE,
  assertLegacyFamilyBusinessScenarioCoverage,
  assertUi01EntryExecutionQueue,
  assertUi01HomeFeatureCoverage,
  assertLegacyFamilyUiArchitectureCoverage,
...
describe('Family Growth OS six-loop UI architecture', () => {
  it('maps all 34 supplied UI screens exactly once', () => {
    expect(() => assertLegacyFamilyUiArchitectureCoverage()).not.toThrow();
    expect(LEGACY_FAMILY_UI_ARCHITECTURE_BINDINGS).toHaveLength(34);
    expect(new Set(LEGACY_FAMILY_UI_ARCHITECTURE_BINDINGS.map((item) => item.ui_id)).size).toBe(34);
  });

  it('uses only the six supplied business-loop families', () => {
    expect(new Set(LEGACY_FAMILY_UI_ARCHITECTURE_BINDINGS.map((item) => item.loop))).toEqual(new Set(LEGACY_FAMILY_SURFACE_LOOPS));
  });

  it('decomposes six PDCA scenarios that collectively cover all 34 UI screens', () => {
    expect(() => assertLegacyFamilyBusinessScenarioCoverage()).not.toThrow();
    expect(LEGACY_FAMILY_BUSINESS_SCENARIOS).toHaveLength(6);
    expect(new Set(LEGACY_FAMILY_BUSINESS_SCENARIOS.map((scenario) => scenario.loop))).toEqual(new Set(LEGACY_FAMILY_SURFACE_LOOPS));
    expect(new Set(LEGACY_FAMILY_BUSINESS_SCENARIOS.flatMap((scenario) => scenario.ui_ids)).size).toBe(34);
  });
  ...
  it('preserves the first real slice and no-external-effect boundary', () => {
    expect(getLegacyFamilyUiArchitectureBinding('UI-09')).toMatchObject({
      loop: 'GROWTH_LOOP', state_boundary: 'NAMED_ACTION', evidence_boundary: 'NAMED_ACTION',
    });
    expect(getLegacyFamilyUiArchitectureBinding('UI-21')).toMatchObject({
