...
/** @param {TenantScopedUiProjection|null} projection @param {'unavailable'|'loading'|'live'} caseAccessState @param {{ projection: { case_id: string, family_id: string, status: string, opened_at?: string, next_action_at?: string|null }, granted_scope: Record<string, unknown> }|null} [caseAccess] @param {'unavailable'|'loading'|'live'} [taskState] @param {Array<{ task_id: string, task_key: string, title: string, description: string, status: string, responsible_ref?: string|null, due_at?: string|null, deliverable?: Record<string, unknown>|null, verified_at?: string|null }>} [tasks] */
function services(projection, caseAccessState = 'unavailable', caseAccess, taskState = 'unavailable', tasks = []) {
  ...
  const taskPanel = taskState === 'live'
    ? `<article class="panel wide" data-service-task-state="READY"><span class="eyebrow">DEV 服务任务</span><h3>交付协作状态</h3>${tasks.length ? `<div class="task-cards">${tasks.map((task) => `<div><small>${escapeHtml(task.status)} · ${escapeHtml(task.task_key)}</small><b>${escapeHtml(task.title)}</b><span>${escapeHtml(task.responsible_ref ?? '尚未分配责任人')} · ${task.verified_at ? '已验收' : '等待交付与验收'}</span><div class="task-actions">${task.status === 'PENDING' ? `<button type="button" data-task-action="assign" data-task-id="${escapeHtml(task.task_id)}">分配给当前顾问</button>` : ''}${['ACCEPTED','IN_PROGRESS'].includes(task.status) ? `<button type="button" data-task-action="deliver" data-task-id="${escapeHtml(task.task_id)}">提交交付记录</button>` : ''}${task.status === 'DELIVERED' ? `<button type="button" data-task-action="verify" data-task-id="${escapeHtml(task.task_id)}">通过验收</button>` : ''}</div></div>`).join('')}</div>` : '<p class="muted">当前 Case 尚未创建协作任务。</p>'}<p class="muted">任务状态是履约协作记录，不等于儿童成长结果；当前不会触发支付、结算或外部通知。</p></article>`
    : `<article class="panel wide" data-service-task-state="${taskState === 'loading' ? 'LOADING' : 'BLOCKED'}"><span class="eyebrow">DEV 服务任务</span><h3>${taskState === 'loading' ? '正在读取交付任务' : '当前没有可读取的交付任务'}</h3><p class="muted">${taskState === 'loading' ? '正在读取当前家庭 Case 的任务状态。' : '需要有效家庭会话与 Case 上下文；默认不生成本地任务。'}</p></article>`;
  return `<section class="workspace-grid"><article class="panel"><span class="eyebrow">服务记录</span><h3>家庭范围内的受控回读</h3><div class="supply-list">${recordsHtml}</div></article>${casePanel}${taskPanel}<article class="panel"><span class="eyebrow">服务边界</span><h3>读取不等于真实履约</h3><div class="quality-row"><b>${projection ? records.length : '—'}</b><span>当前家庭服务记录<br/><small>不是效果评估</small></span></div><div class="quality-row"><b>0</b><span>外部预约与通知<br/><small>本环境一律不执行</small></span></div></article></section>`;
}
...
export function createPlatformConsole(root, options = {}) {
  ...
  let caseAccess = null;
  /** @type {'unavailable'|'loading'|'live'} */ let taskState = options.loadServiceTasks ? 'loading' : 'unavailable';
  /** @type {Array<{ task_id: string, task_key: string, title: string, description: string, status: string, responsible_ref?: string|null, due_at?: string|null, deliverable?: Record<string, unknown>|null, verified_at?: string|null }>} */
  let tasks = [];
  /** @type {'live'|'loading'|'preview'} */
  let receiptState = receiptProjection ? 'live' : options.loadFamilyOperations ? 'loading' : 'preview';
  /** @type {ReceiptFilters} */
  let receiptFilters = { page: 'ALL', status: 'ALL', from: '', to: '', source: 'ALL', authorization: 'ALL', sort: 'NEWEST' };
  ...
  const render = () => {
    ...
    root.querySelector('#quickAction')?.addEventListener('click',()=>{
      ...
      options.createServiceTask({ blueprint_ref: 'DEV_21DAY_SERVICE', task_key: `manual-${Date.now()}`, title: '家庭支持安排', description: '内部 DEV 协作任务：等待责任人交付与质量验收。' })
        .then(async () => { if (options.loadServiceTasks) tasks = await options.loadServiceTasks(); taskState = 'live'; render(); })
        .catch(() => window.alert('DEV 任务创建失败，请检查当前家庭会话与权限。'));
    });
    root.querySelectorAll('[data-task-action]').forEach((button) => button.addEventListener('click', async () => {
      const taskId = /** @type {HTMLElement} */ (button).dataset.taskId ?? '';
      const action = /** @type {'assign'|'deliver'|'verify'} */ (/** @type {HTMLElement} */ (button).dataset.taskAction ?? '');
      try {
        if (action === 'assign' && options.assignServiceTask) await options.assignServiceTask(taskId, { assignee_ref: 'CURRENT_SERVICE_ADVISOR', assignee_kind: 'COACH' });
        if (action === 'deliver' && options.deliverServiceTask) await options.deliverServiceTask(taskId, { deliverable: { delivery_mode: 'DEV_RECORD', note: '已提交内部交付记录，等待人工验收。' } });
        if (action === 'verify' && options.verifyServiceTask) await options.verifyServiceTask(taskId, { reviewer_ref: 'CURRENT_SERVICE_ADVISOR', quality_state: 'PASSED', review_note: 'DEV 受控任务验收通过。' });
        if (options.loadServiceTasks) tasks = await options.loadServiceTasks(); taskState = 'live'; render();
      } catch { window.alert('任务状态更新失败，请检查当前家庭会话、角色权限与任务状态。'); }
    }));
  };
  render();
  ...
  if (options.loadServiceTasks) {
    options.loadServiceTasks().then((next) => { tasks = next; taskState = 'live'; render(); }).catch(() => { tasks = []; taskState = 'unavailable'; render(); });
  }
}
