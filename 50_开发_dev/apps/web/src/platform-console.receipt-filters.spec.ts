...
describe('运营控制台家庭回执筛选', () => {
  ...
  it('supports quick dates, saved filters, time pagination and a server-backed follow-up note without client-side authorization', async () => {
    ...
    const updateFollowUp = vi.fn(async () => ({ follow_up_status: 'PENDING_FOLLOW_UP', operator_note: '请顾问在本周内回看。', follow_up_updated_at: '2026-08-22T10:00:00.000Z', assigned_to_account_id: 'operator-1', assigned_to_display_name: '顾问小林', follow_up_due_date: '2000-01-01' }));
    ...
    const batchAssign = vi.fn(async (operationIds: string[], assignedToAccountId: string, dueDate: string | null) => ({ operation_ids: operationIds, updated_count: operationIds.length, assigned_to_account_id: assignedToAccountId, follow_up_due_date: dueDate }));
    ...
    createPlatformConsole(root, { initialProjection: { tenant: { tenant_id: 'tenant-test', tenant_ref: 'TEST', display_name: '测试租户' } }, initialOperations: { operations }, initialFollowUpAssignees: [{ account_id: 'operator-1', display_name: '顾问小林' }], initialFollowUpWorkspaceMetrics: { today_new: 1, pending: 1, processed: 0, overdue: 1, assignee_workload: [{ account_id: 'operator-1', display_name: '顾问小林', pending_count: 1, overdue_count: 1 }] }, updateFamilyOperationFollowUp: updateFollowUp, batchProcessFamilyOperationFollowUps: batchProcess, batchAssignFamilyOperationFollowUps: batchAssign });
    root.querySelector<HTMLButtonElement>('[data-page="operations"]')?.click();
    ...
    expect(root.textContent).toContain('顾问小林 1');
    ...
    root.querySelector<HTMLSelectElement>('#receiptBatchAssignee')!.value = 'operator-1';
    root.querySelector<HTMLInputElement>('#receiptBatchDueDate')!.value = '2000-01-01';
    root.querySelector<HTMLButtonElement>('#receiptBatchAssign')?.click();
    await new Promise((resolve) => setTimeout(resolve, 0));
    expect(batchAssign).toHaveBeenCalledWith([operationId], 'operator-1', '2000-01-01');
    expect(root.textContent).toContain('已将 1 条家庭回执分派给负责人。');
    root.querySelector<HTMLInputElement>(`[data-operation-select="${operationId}"]`)?.click();
    root.querySelector<HTMLButtonElement>('#receiptOverdue')?.click();
    expect(root.querySelector<HTMLButtonElement>('#receiptOverdue')?.className).toContain('active');
