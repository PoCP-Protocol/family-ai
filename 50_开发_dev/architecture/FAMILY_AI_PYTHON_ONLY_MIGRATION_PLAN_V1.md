...
**Shard-key reservation (2026-08-28, project owner override — borrow the large-scale-platform
shard-key-first discipline, not physical sharding itself):** every domain table's shard key is `family_id`
(never `tenant_id` as primary — a family's data must never split across shards). This is a documentation
commitment now, not an implementation — see `architecture/SHARDING_KEY_REGISTRY.md` for the per-table
registry. Physical sharding (routing layer, shard-aware connection pool) is explicitly deferred until real
production read/write QPS data justifies the operational cost; do not implement a shard router speculatively.
...
**Read/write separation + caching (concrete rule, 2026-08-28):** cache reads, never writes. Mutation
handlers never receive a cache dependency — idempotency-key replay, advisory locks, and audit writes must
stay strongly consistent. Read-side caching applies only to the highest-frequency read projections (e.g.
Assessment's UI-02/UI-03 projections), short TTL, passive expiry (no active invalidation on write) —
see `architecture/SHARDING_KEY_REGISTRY.md` for the concrete justification and
`backend/domains/assessment/infrastructure/cached_query_handler.py` for the reference implementation.
Primary/replica DB topology is a deployment decision, not a code decision, and is not implemented until
real read QPS data justifies it.

**Feed/wide-table pattern (recorded, not applied to Assessment):** the Feed-style denormalized wide-table
pattern common in large-scale content platforms is a good fit for the family growth timeline
(`growth_actions` and related event streams, Batch 2's scope) — reverse-chronological reads over
heterogeneous event types — but not for Assessment's strongly structured session/response relational model.
Recorded in `architecture/SHARDING_KEY_REGISTRY.md` as a design note for whoever picks up Batch 2.
