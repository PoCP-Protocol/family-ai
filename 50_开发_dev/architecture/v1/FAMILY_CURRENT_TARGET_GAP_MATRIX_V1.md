...
| Scenario method | 局部按页面和功能推进 | 所有工作先归入 8 个场景 | 场景到 4A 的代码契约不足 | 在 contracts 增加 `FAMILY_CONSUMER_UI_SCENARIO_4A_MAPPING` |
...
| AA | Mobile/Web 已共享 consumer UI registry | 页面只承载场景，不拥有业务真相 | 历史 legacy UI 材料已移入 reports/legacy-legacy UI | consumer UI baseline contract 已建立 |
