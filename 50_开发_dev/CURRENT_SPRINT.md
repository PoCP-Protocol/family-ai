...
**已登记 (2026-08-27):** A corresponding registry entry has been added as `governance/AUTHORIZATION_REGISTRY.yaml` capability `PROJECT_OWNER_OVERRIDE_SERVICE_COLLAB_ALLOCATION_G1B_PLUS_DB_SCHEMA_20260826` (status `PROJECT_OWNER_OVERRIDE_RECORDED_NOT_CHIEF_ARCHITECT_RATIFIED`). This closes the "no corresponding entry yet" gap noted above; it is a faithful transcription, not a new or expanded authorization, and Chief Architect ratification is still `NOT_YET_OBTAINED`.
