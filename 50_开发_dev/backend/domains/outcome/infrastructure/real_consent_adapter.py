"""Real Consent adapter for the Outcome domain.

Unlike GrowthPriority (which declares a standalone `ConsentCheckPort`),
`OutcomeRepositoryPort.assert_required_growth_consents` is one method
...
`OutcomeRepository` (SQLAlchemy-backed) can compose it in rather than
re-implementing the consent check inline..
"""
